...
export default async function ModonStatsPage() {
  ...
  const deliveryRate =
    totalSentToModon > 0
      ? Math.round(((counts.completed ?? 0) / totalSentToModon) * 100)
      : 0;
  const returnRate =
    totalSentToModon > 0
      ? Math.round(((counts.returned ?? 0) / totalSentToModon) * 100)
      : 0;
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KPI
          ...
          color={deliveryRate >= 70 ? "green" : "red"}
        />
        <KPI
          label="نسبة الإرجاع"
          value={`${returnRate}%`}
          icon={<TrendingDown className="w-4 h-4" />}
          color={returnRate <= 15 ? "green" : "red"}
        />
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {Object.entries(STATUS_META).map(([key, m]) => (
            ...
            >
              ...
              <span className={`text-2xl font-bold ${m.text}`}>
                {counts[key] ?? 0}
              </span>
            </Link>
          ))}
      ...
      <Section title="مقارنة مع نظام مدن">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <MissingCard
            title="مفقودة من مدن"
            subtitle="عندنا باركود — مدن لا تعرفها"
            count={missingInModon.length}
          >
            {missingInModon.length === 0 ? (
              <OK />
            ) : (
              <ul className="divide-y text-sm max-h-48 overflow-y-auto">
                {(missingInModon as any[]).map((o) => (
                  ...
                  >
                    ...
                    <Link
                      href={`/admin/orders?query=${o.modonQrId}&status=pending`}
          ...
          <MissingCard
            title="مفقودة من نظامنا"
            subtitle="موجودة في مدن — لا سجل لها عندنا"
      ...
      {priceStats.total > 0 && (
        <Section title={`مقارنة الأسعار — ${priceStats.total} طلب مكتمل`}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <PriceStat
              label="مطابق"
              value={priceStats.exact}
              color="text-green-700 dark:text-green-400"
            />
            <PriceStat
              label="أقل من المتوقع"
              value={priceStats.less}
              color="text-red-600 dark:text-red-400"
              sub={`-${formatCurrency(priceStats.lossAmount)}`}
            />
            <PriceStat
              label="أكثر من المتوقع"
              value={priceStats.more}
              color="text-amber-600 dark:text-amber-400"
              sub={`+${formatCurrency(priceStats.gainAmount)}`}
            />
            <PriceStat
              label="الفرق الصافي"
...
function MissingCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 space-y-3 ${hasProblem ? "border-red-200 bg-red-50/40 dark:border-red-800 dark:bg-red-950/20" : "bg-card"}`}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div
          className={`flex items-center gap-1 shrink-0 ${hasProblem ? "text-red-600 dark:text-red-400" : "text-green-700 dark:text-green-400"}`}
        >
          {hasProblem ? (
