  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden bg-[#F8F9FA] text-slate-900 font-sans selection:bg-slate-900 selection:text-white">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-20 lg:pt-16 lg:pb-32">
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 lg:sticky lg:top-8 space-y-6">
            ...
            {images.length > 1 && (
              <div
                className={`flex gap-3 overflow-x-auto pb-2 scrollbar-hide ${locale === "ar" ? "pr-1" : "pl-1"}`}
              >
                {images.map((img, idx) => (
                  <button
                    key={idx}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4">
              {[
                ...
                <div
                  key={idx}
                  className="flex flex-col items-center text-center p-3 rounded-2xl bg-white/60 border border-white/60 shadow-sm backdrop-blur-sm"
                >
                  ...
                  {Number(product.shippingPrice) === 0 ? (
                    ...
                    <span className="font-medium text-slate-600 flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-md border border-slate-100">
                      ...
                      <span className="text-slate-900 font-semibold">
                        {formatCurrency(Number(product.shippingPrice))}
                      </span>
                    </span>
                  )}
                </div>
                ...
                {product.offers && (
                  <div className="flex flex-wrap items-center gap-3 text-sm">
                    {product.offers.split(",").map((offer, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg font-medium border border-red-100"
                      >
      ...
      <footer className="relative z-10 py-8 text-center text-slate-400 text-sm font-light">
        <p>
          &copy; {new Date().getFullYear()} AURUM Luxury Store. Excellence in
          every detail.
        </p>
      </footer>
      ...
      <div>
        <a
          ...
          className="fixed bottom-6 left-6 z-50 w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-transform duration-200 hover:scale-110 active:scale-95"
